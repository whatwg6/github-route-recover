...
export function ImageView() {
  ...
  return (
    ...
    >
      <Link key={image.id} to={`subimg/${image.id + 1}`}>
        <div
          style={{
            display: 'grid',
...
export function SubImageView() {
  let navigate = useNavigate();
  let { iid } = useParams<'iid'>();
  let image = getImageById(Number(iid));
  let buttonRef = React.useRef<HTMLButtonElement>(null);
  ...
  if (!image) {
    throw new Error(`No image found with id: ${iid}`);
  }
